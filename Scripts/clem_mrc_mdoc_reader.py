...
class MRCReader:
    ...
    def _rotate_point_about_center(self, x, y, angle_deg, w, h):
        cx = (w - 1) / 2.0
        cy = (h - 1) / 2.0
        ...
        xr = cx + np.cos(th) * dx - np.sin(th) * dy
        yr = cy + np.sin(th) * dx + np.cos(th) * dy
        return xr, yr


    def _rotate_image_for_display(self, img, angle_deg):
        ...
        return rotate(img, angle, reshape=False, order=0, mode="constant", cval=0.0, prefilter=False,)
    ...
    # ------------------------------------------------------------------ #
    # Run MRC Reader and Picker
    # ------------------------------------------------------------------ #
    def run_mrc_reader_and_picker(self):
        self.create_montage()
        self.refine_tile_alignment()
        self.show()
        self.picks = self.pick_stage_positions()

    # ------------------------------------------------------------------ #
    # Display
    # ------------------------------------------------------------------ #
    def show(self, annotate=True, rotated=True, contrast_percentiles=(1.0, 99.8), apply_mdoc_rotation=True):
        ...
        if apply_mdoc_rotation and tiles is not None:
            angle = self._section_rotation_angle(tiles)
            disp_img = self._rotate_image_for_display(disp_img, angle)
    ...
    # Picker
    # ------------------------------------------------------------------ #
    def _assemble_rotated_montage_for_picker(self, img_h, img_w, feather_px, key):
        from scipy.ndimage import rotate
        ...
        max_x = max(x + img_w for _, x, _ in coords)
        max_y = max(y + img_h for _, _, y in coords)

        mont_w = int(np.ceil(max_x - min_x))
        mont_h = int(np.ceil(max_y - min_y))
        ...
        with mrcfile.open(self.path, mode="r", permissive=True) as mrc:
            data = mrc.data
            tiles = {z: data[z].astype(np.float64) for z in z_indices if z < data.shape[0]}
        ...
        for p, x, y in coords:
            ...
            tile = rotate(tiles[z], angle, reshape=False, order=1,
                        mode="constant", cval=0.0, prefilter=False)
            local_wmap = rotate(wmap, angle, reshape=False, order=1,
                                mode="constant", cval=0.0, prefilter=False)

            dx = int(round(x - min_x))
            dy = int(round(y - min_y))

            canvas[dy:dy + img_h, dx:dx + img_w] += tile * local_wmap
            weights[dy:dy + img_h, dx:dx + img_w] += local_wmap
        ...
        return self._normalize_image(canvas.astype(np.float32)), tile_info

    def pick_stage_positions(self, max_px=2000, contrast_percentiles=(1.0, 99.8), apply_mdoc_rotation=True):
        import matplotlib.pyplot as plt
        ...
        img, tiles = self._assemble_rotated_montage_for_picker(
    *self._img_hw,
    self._feather_px,
    key,
)
        ...
        disp_raw = img[::ds, ::ds] if ds > 1 else img

        display_angle = 0.0
        if apply_mdoc_rotation:
            display_angle = self._section_rotation_angle(pieces)
            disp_raw = self._rotate_image_for_display(disp_raw, display_angle)

        disp = self._auto_brightness_contrast(disp_raw, contrast_percentiles, ignore_zeros=True, ignore_whites=True,)
        img_h, img_w = self._img_hw
        pix_um = self.pixel_spacing_um
        ...
        def on_click(event):
            if event.button != 1 or event.inaxes is not ax or event.xdata is None:
                return
            click_px, click_py = event.xdata, event.ydata

            if apply_mdoc_rotation:
                px, py = self._rotate_point_about_center(click_px, click_py, -display_angle, w, h)
            else:
                px, py = click_px, click_py
            ...
            ax.plot(click_px, click_py, "+", color="cyan", markersize=12, markeredgewidth=1.5)
            ax.text(click_px + 8, click_py - 8, str(len(picks)), color="cyan", fontsize=9, fontweight="bold")
