...
Airy images that sit at the same stage position (within POSITION_TOL_UM) are
grouped into one entry.  Every name is kept: the row label lists them joined by
" / ", and the exported CSV carries one row per position with that joined name
plus an n_images count.
...
# Two Airy images whose stage positions differ by less than this are treated as
# the same place. Atlas5 writes float micrometres, so exact equality is unsafe.
POSITION_TOL_UM = 0.5
...
def merge_positions(entries, tol_um=POSITION_TOL_UM):
    """Group Airy entries that sit at the same stage position.

    One entry per position, carrying every name found there in `names` (and a
    joined `name` for display), so nothing is lost when several Airy images
    share a location.
    """
    merged = []
    for e in entries:
        for m in merged:
            if (abs(e["stage_x"] - m["stage_x"]) <= tol_um and
                    abs(e["stage_y"] - m["stage_y"]) <= tol_um):
                m["names"].append(e["name"])
                m["files"].append(e["file"])
                m["fov_um"] = max(m["fov_um"], e["fov_um"])
                break
        else:                                   # no match -> a new position
            merged.append(dict(e, names=[e["name"]], files=[e["file"]]))
    for m in merged:
        m["names"] = list(dict.fromkeys(m["names"]))     # de-dup, keep order
        m["name"]  = " / ".join(m["names"])
    return merged


def read_a5proj(path, merge=True):
    ...
    return [dict(e, names=[e["name"]], files=[e["file"]]) for e in out]
...
class App(tk.Tk):
    ...
    def _build_right(self, p):
        ...
        # Name is wide because positions shared by several Airy images show
        # every name, joined by " / ".
        for c,w in zip(cols,[170,100,100]):
    ...
    def _do_a5(self, path):
        try:
            ...
            n_names = sum(len(a["names"]) for a in self._airy)
            extra = (f"  ({n_names} images grouped)"
                     if n_names != len(self._airy) else "")
            self._sv.set(f"a5proj loaded — {len(self._airy)} positions{extra}")
        except Exception as e:
    ...
    def _update_info(self):
        ...
        if self._airy:
            n_names = sum(len(a["names"]) for a in self._airy)
            if n_names != len(self._airy):
                parts.append(f"a5proj ✓ {len(self._airy)} positions "
                             f"({n_names} images)")
            else:
    ...
    def _compute(self):
        ...
        for a in self._airy:
            ...
            self._results.append(dict(
                ...
                names=a.get("names", [a["name"]]),
                files=a.get("files", [a["file"]]),
        ...
        self._redraw()
        n_names = sum(len(r["names"]) for r in self._results)
        extra = (f" ({n_names} images)" if n_names != len(self._results) else "")
        self._sv.set(
            f"Computed {len(self._results)} positions{extra}. "
    ...
    def _export(self):
        ...
        try:
            ...
            with open(path,"w",newline="") as fh:
                ...
                # w.writerow([])
                w.writerow(["name","grid_x_um","grid_y_um","n_images"])
                for r in self._results:
                    # One row per position; every name that shares it is kept
                    # in the name cell, joined with " / ".
                    w.writerow([r["name"],
                                f"{r['grid_x']:+.3f}", f"{r['grid_y']:+.3f}",
                                len(r.get("names", [r["name"]]))])
            n_names = sum(len(r.get("names", [r["name"]])) for r in self._results)
            messagebox.showinfo("Exported",
                f"Saved {len(self._results)} position(s) "
                f"covering {n_names} image(s) to:\n{path}")
        except Exception as e:
