...
class CLEMCorrelator:
    ...
    def _resolve_flip(self, flip_value, flip_name):
        # The MRC montage is always stored Y-flipped relative to the fluorescence
        # TIFF, so the Y reflection is a permanent, non-optional part of every
        # TIFF->MRC transform. Force it on regardless of what the caller passes
        # (the old code honoured an explicit flip_y=False from the UI, which is
        # what silently broke the similarity fit).
        if flip_name == "flip_y":
            return True
    ...
    def _image_center(tiff_shape):
        """Centre pixel of the TIFF, or None if the shape is unknown. The centre
        is invariant under the X/Y reflection, so it is the same point in the
        original and reflected frames."""
    ...
    def _rescale_matrix(self, M, new_scale, src_anchor, dst_anchor=None):
        """Return a copy of similarity matrix ``M`` whose scale is replaced by
        ``new_scale`` while its rotation/reflection is kept. Translation is
        recomputed so ``src_anchor`` still maps to ``dst_anchor`` (defaulting to
        wherever ``M`` currently sends ``src_anchor``), so the overlay is rescaled
        about that anchor instead of jumping. Returns (matrix, stored_scale)."""
        if new_scale <= 0:
    ...
    def fit_tiff_to_mrc(self, point_pairs, transform_type, predefined=None, predefined_transform=None,
        ...
        """Fit a TIFF->MRC transform. Single entry point for every case:

          * predefined_transform given -> use it as the base (no fitting, and no
            point-pairs required). If fixed_scale is given, the predefined
            transform's scale is replaced by it (rescaled about the point-pair
            centroid if pairs are supplied, otherwise about the image centre).
          * similarity / euclidean      -> reflection-aware similarity fit; if
            fixed_scale is given the scale is bounded to +/- scale_tolerance
          * affine / projective         -> fit directly on the reflected points

        The mandatory reflection F is always composed in, so the returned
        transform maps original (un-reflected) TIFF pixel coords -> MRC coords.
        """
        ...
        base, extra = self._estimate_base(
            transform_type, reflected, dst,
            fixed_scale=fixed_scale, scale_tolerance=scale_tolerance,
            predefined_transform=predefined_transform, initial_transform=initial_transform,
    ...
    def _estimate_base(self, transform_type, reflected, dst, fixed_scale=None, scale_tolerance=0.05,
        ...
        """Return (base_matrix_3x3, extra_fit_info) for the reflected -> dst fit.
        The caller composes 'base' with the reflection F."""

        # 1. A transform was handed to us -> use it as-is, or rescale it.
        if predefined_transform is not None:
            base = np.asarray(predefined_transform.params, dtype=float)
            if fixed_scale is None:
                return base, {}
            # Replace the stored scale with fixed_scale. Anchor about the point-
            # pair centroids when pairs are given, else about the image centre.
            if len(reflected):
        ...
        # 2. affine / projective already contain reflections -> fit directly.
        ...
        # 3. similarity / euclidean, free scale.
        free = np.asarray(estimate_transform("similarity", reflected, dst).params, dtype=float)
        if fixed_scale is None:
            return free, {}

        # 4. similarity, scale bounded to fixed_scale +/- scale_tolerance.
        return self._bound_similarity_scale(free, reflected, dst, fixed_scale,
                                            scale_tolerance, initial_transform)
    ...
    def apply_transform(self, point_pairs, transform_type, tiff_stack, mrc_shape, flip_x=None, flip_y=None, status_cb=None,
        ...
        # The transform is always auto-stored right after it is produced, in the
        # most suitable format, with the creation date-time in the filename.
        if auto_save:
            try:
    ...
    def apply_loaded_transform(self, record, tiff_stack, mrc_shape, point_pairs=None,
        ...
        """Re-apply a previously saved transform to a (possibly different) image.

        Two modes:
          * ``point_pairs is None``  -> warp the new image directly with the
            stored matrix (no re-fit).  Pass ``fixed_scale`` to rescale the
            stored transform to a different scale about the image centre.
          * ``point_pairs`` given    -> re-fit landmarks on the new image; when
            ``scale_limited`` (default) the scale is bounded to ``fixed_scale``
            within ``scale_tolerance`` so only rotation/translation are
            re-solved.  ``fixed_scale`` defaults to the stored transform's scale,
            but pass any value to lock the re-fit to a *different* scale (e.g.
            new optics/binning); ``scale_tolerance`` falls back to the record's
            own tolerance, then 0.05.  This reuses the bounded-scale
            reflected-similarity fit.
        """
